...
> 프리미엄 온라인 화상 소셜 클럽. 그룹 테이블 기반, AI 웨이터 매칭, 마스크 착용 화상, 상호 동의 얼굴 공개.
...
**모달 라우트(인터셉팅 아님, 컴포넌트로 구현)**: 14. 얼굴 공개 동의 모달, 15. 신고·차단 모달.
...
| 테이블 | 핵심 컬럼 |
|---|---|
| `video_sessions` | `id`, `match_id`, `state(live\|paused\|ended\|locked)`, `started_at`, `ended_at`, `provider`, `room_ref` |
...
| `reveal_requests` | `id`, `session_id`, `requester_id`, `target_id`, `state`, `created_at`, `resolved_at` |
| `reveal_permissions` | `session_id`, `user_a_id`, `user_b_id` (정렬 저장), `state`, `granted_at`, `revoked_at`, `revoked_by` | **쌍(pair) 단위 대칭 권한** |
...
쌍(A,B) 단위로 독립적으로 존재하며, 다른 참가자에게는 영향이 없습니다.
...
```
        ...
        │           ▼          ▼            │
        │  ┌──────────────────┐  ┌───────────────────────┐
        │  │ REVEAL_CANCELLED │  │ MUTUAL_REVEAL_PENDING │
        │  └──────────────────┘  └───────────────────────┘
        │           │                       │ 서버 양측 동의 확정
        │           └───────────────────────┤
        │                                   ▼
...
**불변식**
1. 공개는 **양측 명시적 동의**가 서버에 기록된 뒤에만 발생하며, 양측 동시에 적용됩니다.
2. 공개 범위는 해당 **쌍에 한정** — 나머지 참가자에게는 계속 마스크.
3. 어느 한쪽이 마스크를 복구하면 **양방향** 모두 즉시 마스크 상태로 돌아갑니다.
4. 차단·신고·퇴장·세션 종료는 즉시 모든 공개 권한을 취소합니다.
5. 얼굴 추적 실패 시 **절대 원본 얼굴을 노출하지 않고** 블러/프리즈/아바타로 대체합니다.
